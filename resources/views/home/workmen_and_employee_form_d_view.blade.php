    ...
    <div class="col m-3 p-3">
      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormD">
             ...
             <div class="tab" id="tab1">
              ...
              <p>{{isset($user->name) ? $user->name : $cat->name}} <br>{{(isset($user->address) ? $user->address : $cat->address)}} </p>
              ...
              <p class="text-dark"> <span id="name_user">{{isset($user->name) ? $user->name : $cat->name}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>


                <div class="form-group">
                <label>1. Name of WORKMAN / EMPLOYEE </label> <span style="color:red">*</span>
                <br><span id="alert_name" style="color:red"></span>
                <p><input class="form-control" id="name" name="name" type="text" value="{{isset($user->name) ? $user->name : $cat->name}}" placeholder="Workman / Employee Name..." readonly="true"></p>
                ...
                <p><input class="form-control"  id="address" value="{{(isset($user->address) ? $user->address : $cat->address)}}" name="address" type="text" placeholder="Address..." ></p>
                ...
                <p><input name="email" id="email" value="{{(isset($user->email) ? $user->email : $cat->email)}}" class="form-control" type="email" placeholder="Email ..."  readonly="true"></p></div>
               ...
               <input class="form-control" id="principle" value="{{(isset($user->principle) ? $user->principle : '')}}" name="principle" type="numbers" placeholder="Principle ..." >

               <br>
                <input class="form-control" id="interest"  value="{{(isset($user->intrest) ? $user->intrest : '')}}" name="interest" type="numbers" placeholder="Interest ..." ></div>

                <input class="form-control" id="total_amount" value="{{(isset($user->total_amount) ? $user->total_amount : '')}}" name="total_amount" type="numbers" placeholder="Total Amount ..." >
                <br>
                <div class="form-group">
                ...
                <input id="block_letter" name="name_block_letter" value="{{isset($user->name_in_block_letter) ? strtoupper($user->name) : strtoupper($cat->name)}}" class="form-control" type="text"  placeholder="Enter name in BLOCK LETTERS..." readonly></p>

                <p>Position with or in relation to creditor 
                <br><span id="alert_relation_to_creditor" style="color:red"></span>  <input value="{{(isset($user->relation_to_creditor) ? $user->relation_to_creditor : '')}}" id="relation_to_creditor" name="relation_to_creditor" class="form-control" type="text" placeholder="Enter Details..." ></p>

                <p>Address of person signing 
                <br><span id="alert_address_person_signing" style="color:red"></span>
                <textarea id="address_person_signing" name="address_person_signing"  style="width:100%;" placeholder="Enter Address..." > {{(isset($user->address_person_signing) ? $user->address_person_signing : '')}}</textarea></p>
              ...
              <table id="myTable1">
               @if(isset($other_files) && count($other_files)>0)
                ...
                <tr id="row{{$loop->index+1}}">
                  ...
                  <td><input type='file' name="other_doc_file[]" onchange="updateFile('financialCreditorFormD','/update-formc-file','{{$fl->id}}','{{$loop->index}}', '{{$tbl}}')">
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData('financialCreditorFormD','thirdStep','tab2', 'tab3', check2, 'step2', 'step3', '/form/financial-creditor-form-d-save', 'errMessage_3');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '{{isset($view) ? 'view' : ''}}', '/get-formD-signature', 'step3Sign');" >Next</button>
           ...
           <div class="tab" id="tab3">
             ...
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2');updateTableData('financialCreditorFormD','myTable1','/form/get-formD-updated-table');">Previous</button>
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData2('financialCreditorFormD','thirdStep','tab3', 'tab4', check2, 'step3', 'step4', '/form/financial-creditor-form-d-save', 'errMessage_3');" >Next</button>
           ...
           <div class="tab" id="tab4">
              <h2 class="text-center">FORM D View</h2>
