              ...
              <h6>I {{!empty($user->signing_person_name) ? $user->signing_person_name : ''}}, currently residing at {{isset($user->signing_address) ? $user->signing_address : ''}}, do hereby declare and state as follows: - </h6><br>

              <span>1. {{!empty($comp->name) ? ucwords($comp->name) : ''}}, the corporate debtor was, at the insolvency commencement date, being the {{dateFm3($user->insolvency_commencement_date)}} actually indebted to me for a sum of Rs. {{is_numeric($user->claim_amt) ? $user->claim_amt : ''}}.</span><br><br>
              ...
              @foreach($senctionDetails as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionAmtTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormF', 'errMessage_4', '{{Config::get('site.formFSenctionMdl')}}')">Delete Row</button></td>
                </tr>
              ...
              @foreach($declarationMdls as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionDeclTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormF', 'errMessage_4', '{{Config::get('site.formFDecTblMdl')}}')">Delete Row</button></td>
                </tr>
            ...
            a related party of the corporate debtor, as defined under section 5 (24) of the Code.</span><br><br>
              ...
              <span><img class="profile-pic-user" src="{{ asset('public/access/media/forms/formF/documents/'.$user->unique_id.'/'.$user->fc_signature) }}" width="75" height="75" style="float: right;"></span>
