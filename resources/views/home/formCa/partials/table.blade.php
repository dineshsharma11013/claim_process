 @forelse($other_files as $fl)
                <tr id="row{{$loop->index+1}}">
                  ...
                  <td><input type='file' name="other_doc_file[]" onchange="updateFile('financialCreditorFormCA','/update-formca-file','{{$fl->id}}','{{$loop->index}}', '{{$tbl}}')">
              ...
              <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="del('{{$loop->index+1}}', '{{$fl->id}}', '/remove-formca-data', 'deleteRowBtn', 'financialCreditorFormCA', 'errMessage_4')">Delete Row</button></td>
                </tr>
                @empty
                <tr id="row1">
