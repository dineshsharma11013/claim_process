       ...
       <input type="hidden" name="form_c_selected_id" id="form_c_selected_id" value="{{isset($user->form_c_selected_id) ? $user->form_c_selected_id : ''}}">
        <input type="hidden" name="updated_data" id="updated_data" value="">
            ...
            <div class="tab" id="tab1">
              ...
              <h4>From</h4>
               <p><span id="fc_name"> {{!empty($user->fc_name) ? $user->fc_name : ''}} </span>
                <br>
                <span id="userAddress">
              ...
              <p class="text-dark">I <span id="signing_person_name">{{!empty($user->signing_person_name) ? $user->signing_person_name : $cat->name}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>


                <div class="form-group">
                <label>1. Name of financial creditor <span class="required_cls">*</span></label>
                <p><input class="form-control" name="fc_name" id="fc_name" type="text" value="{{!empty($user->fc_name) ? $user->fc_name : ''}}" placeholder="Financial  Creditor Name..." autocomplete="off" onKeyUp="Removefd('financialCreditorForm','fc_name');getName('financialCreditorForm','fc_name', this.value);" autocomplete="off"></p>
                ...
                <p><input class="form-control" type="text" name="fc_email" id="fc_email" onKeyUp="Removefd('financialCreditorForm','fc_email');" value="{{isset($user->fc_email) ? $user->fc_email : $cat->email}}" placeholder="Email ..." autocomplete="off"></p>
                ...
                <p>Position with or in relation to creditor <input class="form-control" value="{{isset($user->creditor_position) ? $user->creditor_position : ''}}" name="creditor_position" id="creditor_position" type="text" placeholder="Enter Details..." autocomplete="off" onKeyUp="Removefd('financialCreditorForm','signing_address');">
                  <div class="error_cls" id="error_creditor_position"></div>
                </p>

                <p>Address of person signing 
                  <input class="form-control" value="{{isset($user->signing_address) ? $user->signing_address : ''}}" name="signing_address" id="signing_address" type="text" placeholder="Enter Address..." autocomplete="off" onKeyUp="Removefd('financialCreditorForm','signing_address');" >
            ...
            <div class="tab" id="tab2">
                ...
                <div class="form-group">
                <label> LIST OF DOCUMENTS ATTACHED TO THIS PROOF CLAIM IN ORDER TO PROVE THE EXISTENCE AND NON-PAYMENT OF CLAIM DUE TO THE financial creditor</label><br><br>
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData2('financialCreditorForm','thirdStep','tab2', 'tab3', check2, 'step2', 'step3', '/form/financial-creditor-form-c-save', 'errMessage_3','/form/get-formc-updated-table');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '', '/get-formC-signature', 'step3Sign');" >Next</button>
            ...
            <div class="tab" id="tab4">
              <h3 class="text-center">FORM C <br> PREVIEW </h3><br><br>
             ...
             <br><br>
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab4','tab3','step4','step3');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '', '/get-formC-signature', 'step3Sign');">Previous</button>
