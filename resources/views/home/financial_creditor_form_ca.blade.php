        ...
        <input type="hidden" name="form_ca_selected_id" id="form_ca_selected_id" value="{{isset($user->form_ca_selected_id) ? $user->form_ca_selected_id : ''}}">
          <input type="hidden" name="updated_data" id="updated_data" value="">
            ...
            <div class="tab" id="tab1">
              ...
              <h6 class="mb-3">(Under Regulation 8A of the Insolvency and Bankruptcy Board of India (Insolvency Resolution Process for Corporate Persons) Regulations, 2016)</h6>
              <h4 style="float: right;">@if(isset($user)) {{ ($user->submitted == 1) ? dateFm($user->date) : date("d-M-Y")}} @else {{date("d-M-Y")}} @endif</h4>


              <h4>From</h4>

              <p><span id="fc_name_2">{{!empty($user->fc_name) ? $user->fc_name : ''}}</span>
              ...
              <p class="text-dark">I <span id="fc_name_2">{{!empty($user->fc_name) ? $user->fc_name : ''}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                ...
                <input class="form-control" placeholder="Account number ..." onkeypress="return isNumberKey(event);" type="text" value="{{isset($user->bank_account_number) ? $user->bank_account_number : ''}}" name="bank_account_number" id="bank_account_number" autocomplete="off">

                <input class="form-control" placeholder="Bank Name..." type="text" value="{{isset($user->bank_name) ? $user->bank_name : ''}}" name="bank_name" id="bank_name" autocomplete="off">
                ...
                <div class="error_cls" id="error_fc_signature" autocomplete="off"></div>
                </div>
                <br>

                <div><label> Name in BLOCK LETTERS </label><span class="required_cls">*</span> <input class="form-control" name="creditor_name" id="creditor_name" type="text" placeholder="Enter name in BLOCK LETTERS..." value="{{!empty($user->signing_person_name) ? strtoupper($user->signing_person_name) : strtoupper($cat->name) }}" onKeyUp="Removefd('financialCreditorFormCA','creditor_name');" autocomplete="off">
                  <div class="error_cls" id="error_creditor_name"></div>
                </div>
                <br>

                <div><label>Position with or in relation to creditor</label> <span class="required_cls">*</span> <input class="form-control" value="{{isset($user->creditor_position) ? $user->creditor_position : ''}}" type="text" name="creditor_position" id="creditor_position" placeholder="Enter Details..." autocomplete="off" onKeyUp="Removefd('financialCreditorFormCA','creditor_position');" >
                  <div class="error_cls" id="error_creditor_position"></div>
                </div>
                <br>
                <div><label>Address of person signing </label> <span class="required_cls">*</span> <input class="form-control" type="text" name="creditor_address" id="creditor_address" value="{{isset($user->signing_address) ? $user->signing_address : ''}}" placeholder="Address..." autocomplete="off" onKeyUp="Removefd('financialCreditorFormCA','creditor_address');" >
            ...
            <div class="tab" id="tab4">
              <h3 class="text-center">FORM CA PREVIEW </h3>
