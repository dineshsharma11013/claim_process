    ...
    <div class="col m-3 p-3">
      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormD">
             ...
             <div class="tab" id="tab1">
               ...
               <h2 class="text-center">FORM D <!-- {{Session::get('form_a')}}--></h2>
              ...
              <p class="text-dark">I <span id="name">{{isset($user->name) ? $user->name : ''}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                ...
                <p><input class="form-control" id="name" name="name" type="text" value="{{isset($user->name) ? $user->name : ''}}" placeholder="Workman / Employee Name..." onKeyUp="Removefd('financialCreditorFormD','name');getName('financialCreditorFormD','name', this.value)" autocomplete="off"></p>
                ...
                <input class="form-control" id="pancard"  value="{{(isset($user->pancard_no) ? $user->pancard_no : '')}}" type="text" name="pancard" placeholder="Pan Card number ..." autocomplete="off">
                <span id="alert_passport" style="color:red"></span>
                <input class="form-control" id="passport"  value="{{(isset($user->passport_no) ? $user->passport_no : '')}}" name="passport" type="text" placeholder="Passport number ..." autocomplete="off">
                <span id="alert_voter_id" style="color:red"></span>
                <input class="form-control" id="voter_id"  value="{{(isset($user->voter_id_no) ? $user->voter_id_no : '')}}" name="voter_id" type="numbers" placeholder="Voter Id card number ..." autocomplete="off">
                <span id="alert_aadhar" style="color:red"></span>
                <input class="form-control" name="aadhar" id="aadhar"  value="{{(isset($user->aadhar_no) ? $user->aadhar_no : '')}}" type="numbers" placeholder="Aadhar card number ..." autocomplete="off">
              </div>
                ...
                <p><input class="form-control"  id="address" value="{{(isset($user->address) ? $user->address : '')}}" name="address" type="text" placeholder="Address..." onKeyUp="Removefd('financialCreditorFormD','address');getAddress('financialCreditorFormD','address', this.value)" autocomplete="off"></p>
               ...
               <br>
                ...
                <p><input class="form-control" id="details_of_documents" value="{{(isset($user->details_of_documents) ? $user->details_of_documents : '')}}"  name="details_of_documents" type="text" placeholder="Details of documents..." autocomplete="off"></p>
                </div>
                ...
                <p><input class="form-control"  id="dispute" name="dispute" value="{{(isset($user->dispute_details) ? $user->dispute_details : '')}}" type="text" placeholder="Enter Details..." autocomplete="off"></p>
                </div>

                <div class="form-group">
                <label>7. Details of how and when claim arose </label> 
                <br><span id="alert_claim" style="color:red"></span>
                <p><input class="form-control"  id="claim_arose"  name="claim"  value="{{(isset($user->claim_arose_details) ? $user->claim_arose_details : '')}}"  type="text" placeholder="Enter Details..." autocomplete="off"></p>
                </div>
                ...
                <p><textarea name="mutual_credit" id="mutual_credit" style="width:100%;" placeholder="Enter Details..." autocomplete="off">{{(isset($user->mutual_credit_details) ? $user->mutual_credit_details : '')}}</textarea></p>
                </div>
                ...
                <input class="form-control" value="{{(isset($user->account_no) ? $user->account_no : '')}}" name="account_number" id="account_number" placeholder="Account number ..." autocomplete="off">

                <br><span id="alert_bank_name" style="color:red"></span>
                <input class="form-control" id="bank_name" name="bank_name"  value="{{(isset($user->bank_name) ? $user->bank_name : '')}}" placeholder="Bank Name..." autocomplete="off">

                <br><span id="alert_ifsc" style="color:red"></span>
                <input class="form-control" id="ifsc"  name="ifsc"  value="{{(isset($user->ifsc_code) ? $user->ifsc_code : '')}}" placeholder="IFSC Code ..." autocomplete="off"></div>
                ...
                <div>
                <label>Name in BLOCK LETTERS </label> <span class="required_cls">*</span>
                ...
                <div> <label> Position with or in relation to creditor </label> <span class="required_cls">*</span>
                ...
                <br>
                <div> <label> Address of person signing </label> <span class="required_cls">*</span>
           ...
           <div class="tab" id="tab2">
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData2('financialCreditorFormD','thirdStep','tab2', 'tab3', check2, 'step2', 'step3', '/form/financial-creditor-form-d-save', 'errMessage_3','/form/get-formD-updated-table');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '', '/get-formD-signature', 'step3Sign');" >Next</button>
           ...
           <div class="tab" id="tab3">
             ...
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2');">Previous</button>
              ...
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData('financialCreditorFormD','firstStep','tab3', 'tab4', check0, 'step3', 'step4', '/form/financial-creditor-form-d','errMessage_2', getPreview);updateTableData('financialCreditorFormD','myTable1','/form/get-formD-updated-table');" >Next</button>
           ...
           <div class="tab" id="tab4">
             ...
             <br><br>
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab4','tab3','step4','step3');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '');">Previous</button>
 ...
 function handleChange(check, id)
{
  ...
  if (isChecked) 
  {
    $('#financialCreditorFormD input[id="'+id+'"]').show(); 
    //$("#"+id).show();
  }
  else
  {
    $('#financialCreditorFormD input[id="'+id+'"]').hide(); 
   //$("#"+id).hide(); 
  }
