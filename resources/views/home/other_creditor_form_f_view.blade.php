      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormF">
       ...
       <input type="hidden" name="updated_data" id="updated_data" value="{{isset($view) ? 'view' : ''}}">
            ...
            <div class="tab" id="tab1">
              ...
              <h6 class="mb-3">[Under Regulation 9A of the Insolvency and Bankruptcy Board of India (Insolvency Resolution Process for Corporate Persons) Regulations, 2016]</h6>
              <h4 style="float: right;">@if(isset($user)) {{ ($user->submitted == 1) ? dateFm($user->date) : date("d-M-Y")}} @else {{date("d-M-Y")}} @endif</h4>

              <h4>From</h4>

              <p>{{isset($user->fc_name) ? $user->fc_name : ''}}
             <br>   <span id="userAddress">
              ...
              <p class="text-dark">I <span id="creditor_name">{{!empty($user->fc_name) ? $user->fc_name : ''}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                ...
                <div class="form-group">
                <label>2. Identification number of the creditor <br> <span class="text-secondary"> (If an incorporate body, provide of incorporation. If a partnership or individual provide identification records* of all the partners or the individual) </span> </label>
                ...
                <div class="form-group">
                <label>3.</label><br>
                <label class="mr-1"> &nbsp; &nbsp; &nbsp;a. Address of  Creditor for correspondence. <span class="required_cls">*</span></label>
                ...
                <input class="form-control" type="email" name="fc_email" id="fc_email" value="{{isset($user->fc_email) ? $user->fc_email : ''}}" placeholder="Email ..." onKeyUp="Removefd('financialCreditorFormF','fc_email');">
               ...
               <input style="width:20%;float: left;" type="text" name="claim_amt" id="claim_amt" value="{{isset($user->claim_amt) ? $user->claim_amt : ''}}" placeholder="Enter Amount" >
                ...
                <br>
                <p>Name in BLOCK LETTERS <input class="form-control" name="creditor_name" id="creditor_name" type="text" placeholder="Enter name in BLOCK LETTERS..." value="{{!empty($user->signing_person_name) ? strtoupper($user->signing_person_name) : strtoupper($cat->name)}}" readonly="true"></p>

                <p>Position with or in relation to creditor <input class="form-control" value="{{isset($user->creditor_position) ? $user->creditor_position : ''}}" type="text" name="creditor_position" id="creditor_position" placeholder="Enter Details...">
                  <div class="error_cls" id="error_creditor_position"></div>
                </p>

                <p>Address of person signing <input class="form-control" type="text" name="creditor_address" id="creditor_address" value="{{isset($user->signing_address) ? $user->signing_address : ''}}" placeholder="Address..."></p>
            ...
            <div class="tab" id="tab2">
              ...
              <table id="myTable1">
               @if(isset($other_files) && count($other_files)>0)
               
                @foreach($other_files as $fl)
                <tr id="row{{$loop->index+1}}">
                  ...
                  <td><input type='file' name="other_doc_file[]" onchange="updateFile('financialCreditorFormF','/update-formca-file','{{$fl->id}}','{{$loop->index}}', '{{$tbl}}')">
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData2('financialCreditorFormF','thirdStep','tab2', 'tab3', check2, 'step2', 'step3', '/form/financial-creditor-form-f-save', 'errMessage_3', '/form/get-formf-updated-table');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '{{isset($view) ? 'view' : ''}}', '/get-formF-signature', 'step3Sign');" >Next</button>
            ...
            <div class="tab" id="tab3">
             ...
             <br><br>
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2');updateTableData('financialCreditorFormF','myTable1','/form/get-formf-updated-table');">Previous</button>
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData('financialCreditorFormF','secondStep','tab3', 'tab4', check1, 'step3', 'step4', '/form/financial-creditor-form-f', 'errMessage_2', getPreview);" >Next</button>
            ...
            <div class="tab" id="tab4">
              <h2 class="text-center">FORM F PREVIEW </h2>
 ...
 function handleChange(check, id)
{
  ...
  if (isChecked) 
  {
    $('#financialCreditorFormF input[id="'+id+'"]').show(); 
    //$("#"+id).show();
  }
  else
  {
    $('#financialCreditorFormF input[id="'+id+'"]').hide(); 
   //$("#"+id).hide(); 
  }
