    ...
    <div class="col m-3 p-3">
      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormCA">
        ...
        <input type="hidden" name="form_type" value="{{$user_type}}">
          ...
          <input type="hidden" name="ar" id="ar" value="{{isset($user->ar) ? $user->ar : $arV}}">
          ...
          <input type="hidden" name="updated_data" id="updated_data" value="{{isset($view) ? 'view' : ''}}">
            ...
            <div class="tab" id="tab1">
              ...
              <p>{{!empty($user->fc_name) ? $user->fc_name : ''}}
              ...
              <p class="text-dark">I {{!empty($user->fc_name) ? $user->fc_name : ''}}, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                ...
                <input class="form-control" type="text" name="fc_address" id="fc_address" value="{{isset($user->fc_address) ? $user->fc_address : ''}}" placeholder="Address..." onKeyUp="Removefd('financialCreditorFormCA','fc_address');getAddress('financialCreditorFormCA','fc_address', this.value)">
                ...
                <input class="form-control" type="email" name="fc_email" id="fc_email" value="{{isset($user->fc_email) ? $user->fc_email : ''}}" placeholder="Email ..." onKeyUp="Removefd('financialCreditorFormCA','fc_email');">
                ...
                <input class="form-control" type="text" name="claim_principle" id="claim_principle" value="{{isset($user->claim_principle) ? $user->claim_principle : ''}}" onkeypress="return isNumberKey(event);" placeholder="Principle ...">
                
                <input class="form-control" type="text" name="claim_interest" id="claim_interest" value="{{isset($user->claim_interest) ? $user->claim_interest : ''}}" onkeypress="return isNumberKey(event);" placeholder="Interest ...">

                <input class="form-control" type="text" name="claim_amt" id="claim_amt" value="{{isset($user->claim_amt) ? $user->claim_amt : ''}}" onkeypress="return isNumberKey(event);" placeholder="Total Amount ...">
                </div>
                
                <div class="form-group">
                <label>5. Details of documents by references to which can be substantiated. </label>
                <input class="form-control" type="text" name="document_details" id="document_details" value="{{isset($user->document_details) ? $user->document_details : ''}}" placeholder="Details Of Documents ...">
                ...
                <input class="form-control" type="text" name="debt_incurred_details" id="debt_incurred_details" value="{{isset($user->debt_incurred_details) ? $user->debt_incurred_details : ''}}" placeholder="Details example...">
                ...
                <input class="form-control" type="text" name="other_mutual_details" id="other_mutual_details" value="{{isset($user->other_mutual_details) ? $user->other_mutual_details : ''}}" placeholder="Details example...">
                ...
                <input class="form-control" type="text" name="insolvency_professional_name" id="insolvency_professional_name" value="{{isset($user->insolvency_professional_name) ? $user->insolvency_professional_name : ''}}" placeholder="Name Details...">
                ...
                <input class="form-control" type="file" name="fc_signature" id="fc_signature" accept="image/*" onchange="loadFile('fc_signature','error_fc_signature')">

                <input type="hidden" name="fc_signature_val" id="fc_signature_val" value="{{isset($user->fc_signature) ? $user->fc_signature : ''}}">
                <div class="error_cls" id="error_fc_signature"></div>
              </div>
                <br>

                <div>Name in BLOCK LETTERS  <input class="form-control"  id="creditor_name" type="text" placeholder="Enter name in BLOCK LETTERS..." value="{{!empty($user->signing_person_name) ? strtoupper($user->signing_person_name) : strtoupper($cat->name) }}" onKeyUp="Removefd('financialCreditorFormCA','creditor_name');" autocomplete="off">
                  <div class="error_cls" id="error_creditor_name"></div>
                </div>
                <br>

                <div>Position with or in relation to creditor <input class="form-control" value="{{isset($user->creditor_position) ? $user->creditor_position : ''}}" type="text"  id="creditor_position" placeholder="Enter Details..." autocomplete="off" >
                  <div class="error_cls" id="error_creditor_position"></div>
                </div>
                <br>
                <div>Address of person signing  <input class="form-control" type="text"  id="creditor_address" value="{{isset($user->signing_address) ? $user->signing_address : ''}}" placeholder="Address..." autocomplete="off" onKeyUp="Removefd('financialCreditorFormCA','creditor_address');" >
            ...
            <div class="tab" id="tab3">
             ...
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2')">Previous</button>
              ...
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData('financialCreditorFormCA','secondStep','tab3', 'tab4', check1, 'step3', 'step4', '/form/financial-creditor-form-ca', 'errMessage_2', getPreview);" >Next</button>
            ...
            <div class="tab" id="tab4">
              ...
              <div id="step3Sign" style="text-align: center;margin-top: 40px;"></div>
              <br><br>
             <!-- <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab4','tab3','step4','step3');updateTableData('financialCreditorFormCA','myTable1','/form/get-formca-updated-table');">Previous</button> -->
