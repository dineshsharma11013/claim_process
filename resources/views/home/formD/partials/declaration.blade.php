<h6>I, {{!empty($user->name_in_block_letter) ? $user->name_in_block_letter : $cat->name}} currently residing at {{isset($user->address_person_signing) ? $user->address_person_signing : ''}}, do hereby declare and state as follows: - </h6><br>
              ...
              @foreach($senctionDetails as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionAmtTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormD', 'errMessage_4', '{{Config::get('site.formDSenctionMdl')}}')">Delete Row</button></td>
                </tr>
              ...
              @foreach($declarationMdls as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionDeclTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormD', 'errMessage_4', '{{Config::get('site.formDDecTblMdl')}}')">Delete Row</button></td>
                </tr>
            ...
            a related party of the corporate debtor, as defined under section 5 (24) of the Code.</span><br><br>
              ...
              <span><img class="profile-pic-user" src="{{ asset('public/access/media/forms/formD/documents/'.$user->unique_id.'/'.$user->operational_creditor_signature) }}" width="75" height="75" style="float: right;"></span>
