      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormF">
       ...
       <input type="hidden" name="updated_data" id="updated_data" value="">
            ...
            <div class="tab" id="tab1">
              ...
              <p><span id="creditor_name"> {{!empty($user->fc_name) ? $user->fc_name : ''}} </span>
              ...
              <p class="text-dark">I <span id="creditor_name">{{!empty($user->fc_name) ? $user->fc_name : ''}}</span>, hereby submits this claim in respect of the corporate insolvency resolution process of {{isset($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                ...
                <input class="form-control" type="text" name="fc_address" id="fc_address" value="{{isset($user->fc_address) ? $user->fc_address : ''}}" placeholder="Address..." onKeyUp="Removefd('financialCreditorFormF','fc_address');getAddress('financialCreditorFormF', this.value)" autocomplete="off">
                <div class="error_cls" id="error_fc_address"></div>
                </div>

                <div class="form-group">
                <label>&nbsp; &nbsp; &nbsp; b. Email Address <span class="required_cls">*</span></label>
                <input class="form-control" type="email" name="fc_email" id="fc_email" value="{{isset($user->fc_email) ? $user->fc_email : $cat->email}}" placeholder="Email ..." onKeyUp="Removefd('financialCreditorFormF','fc_email');">
               ...
               <input style="width:20%;float: left;" type="text" name="claim_amt" id="claim_amt" value="{{isset($user->claim_amt) ? $user->claim_amt : ''}}" placeholder="Enter Amount" onKeyUp="Removefd('financialCreditorFormF','claim_amt');" onchange="checkAmount('financialCreditorFormF','claim_amt');" >
                ...
                <br>
                <div><label>Name in BLOCK LETTERS</label> <span class="required_cls">*</span> <input class="form-control" name="creditor_name" id="creditor_name" type="text" placeholder="Enter name in BLOCK LETTERS..." value="{{!empty($user->signing_person_name) ? strtoupper($user->signing_person_name) : ''}}" onKeyUp="Removefd('financialCreditorFormF','creditor_name');" autocomplete="off">
                  <div class="error_cls" id="error_creditor_name"></div>
                </div>
                <br>

                <div><label>Position with or in relation to creditor</label> <input class="form-control" value="{{isset($user->creditor_position) ? $user->creditor_position : ''}}" type="text" name="creditor_position" id="creditor_position" placeholder="Enter Details..." autocomplete="off" onKeyUp="Removefd('financialCreditorFormF','creditor_position');" >
                  <div class="error_cls" id="error_creditor_position"></div>
                </div>
                <br>
                <div><label>Address of person signing </label>  <input class="form-control" type="text" name="creditor_address" id="creditor_address" value="{{isset($user->signing_address) ? $user->signing_address : ''}}" placeholder="Address..." autocomplete="off" onKeyUp="Removefd('financialCreditorFormF','creditor_address');" >
            ...
            <div class="tab" id="tab2">
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData2('financialCreditorFormF','thirdStep','tab2', 'tab3', check2, 'step2', 'step3', '/form/financial-creditor-form-f-save', 'errMessage_3','/form/get-formf-updated-table');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '', '/get-formF-signature', 'step3Sign');" >Next</button>
             ...
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2');">Previous</button>
              ...
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData('financialCreditorFormF','firstStep','tab3', 'tab4', check0, 'step3', 'step4', '/form/financial-creditor-form-f','errMessage_2',getPreview);updateTableData('financialCreditorFormF','myTable1','/form/get-formf-updated-table');" >Next</button>
            ...
            <div class="tab" id="tab4">
             ...
             <br><br>
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab4','tab3','step4','step3');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '', '/get-formF-signature', 'step3Sign');">Previous</button>
