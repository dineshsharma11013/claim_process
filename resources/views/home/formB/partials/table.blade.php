 @forelse($other_files as $fl)
                ...
                <tr id="row{{$loop->index+1}}">
                     @php $tbl= Config::get('site.formBOtherDoc'); @endphp
                     <td><input type='text' placeholder='enter document name' value="{{$fl->document_name}}" name="other_doc[]" autocomplete="off"></td>
                  <td><input type='file' name="other_doc_file[]" onchange="updateFile('operationalCreditorForm','/update-formb-file','{{$fl->id}}','{{$loop->index}}', '{{$tbl}}')">
              ...
              <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="del('{{$loop->index+1}}', '{{$fl->id}}', '/remove-formb-data', 'deleteRowBtn', 'operationalCreditorForm', 'errMessage_4')">Delete Row</button></td>
                </tr>
