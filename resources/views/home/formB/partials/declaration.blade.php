<h6> I {{!empty($user->claimant_name) ? ucwords($user->claimant_name) : ucwords($cat->name)}}, currently residing at {{isset($user->signing_person_address) ? $user->signing_person_address : $cat->address}}, do hereby declare and state as follows: - </h6><br>
              ...
              @foreach($senctionDetails as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionAmtTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'operationalCreditorForm', 'errMessage_4', '{{Config::get('site.formBSenctionMdl')}}')">Delete Row</button></td>
                </tr>
              ...
              @foreach($declarationMdls as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionDeclTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'operationalCreditorForm', 'errMessage_4', '{{Config::get('site.formBDecTblMdl')}}')">Delete Row</button></td>
                </tr>
            ...
            a related party of the corporate debtor, as defined under section 5 (24) of the Code.</span><br><br>
              ...
              <div>
                     ...
                     @else
                     <label>Place:</label>
                     <input type="text" name="place" id="place" value="{{isset($user->place) ? $user->place : ''}}" onKeyUp="Removefd('operationalCreditorForm', 'place')" style="margin-left: 10px;width: 250px;" autocomplete="off">
                    <div class="error_cls" id="error_place" style="margin-left: 55px;"></div>
                     @endif
                     
                     
              </div>
              <div class="error_cls" id="error_place" style="margin-left: 50px;"></div> 
              <!-- <h5>Place:</h5> <input type="text" name="place" id="place" value="{{isset($user->place) ? $user->place : ''}}">  -->
              <br><br>
              ...
              <span><img class="profile-pic-user" src="{{ asset('public/access/media/forms/formB/documents/'.$user->unique_id.'/'.$user->operational_creditor_signature) }}" width="75" height="75" style="float: right;"></span>
              ...
              <p>I, {{!empty($user->claimant_name) ? ucwords($user->claimant_name) : ucwords($cat->name)}} the {{!empty($user->creditor_relation) ? $user->creditor_relation : ''}} hereinabove, do hereby verify that the contents of his proof of claim are true and correct to my Knowledge and belief and no material fact has been concealed therefrom.</p><br>
