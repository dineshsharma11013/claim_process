      ...
      <form class="mt-0 border border-info rounded form-body" id="financialCreditorFormE">
       ...
       <input type="hidden" name="updated_data" id="updated_data" value="{{isset($view) ? 'view' : ''}}">
            ...
            <div class="tab" id="tab1">
              ...
              <p class="text-dark">I, {{isset($cat->name) ? $cat->name : ''}}, currently residing at {{isset($cat->address) ? $cat->address : ''}}, on behalf of the workmen and employees employed by the above named corporate debtor and listed in Annexure A, solemnly affirm and say:</p>
...
<table class="table" >
  ...
  <tbody id="myTable2">
    ...
    <tr>
      ...
      <td><input type="text" value="{{$emp->employee_name}}" placeholder="enter details" name="employee_name[]" id="employee_name"></td>
      <td> <select class="form-select" aria-label="Default select example" name="id_number[]" id="id_number" autocomplete="off">
                          <option value="">--Select--</option>
                          @foreach(Config::get('site.identification_type') as $id => $iv)
                            <option value="{{$id}}" {{($emp->id_number == $id) ? 'selected' : '' }}>{{$iv}}</option>
                          @endforeach
                       </select>
                       <input type="text" value="{{$emp->id_details}}" placeholder="enter details" name="id_details" id="id_details">

</td>
      <td><input type="text" name="total_amt[]" id="total_amt" value="{{$emp->total_amt}}" placeholder="enter details"></td>
      <td><input type="text" name="due_amt_period[]" id="due_amt_period" value="{{$emp->due_amt_period}}" placeholder="enter details"></td>
    </tr>
    @endforeach
    @else
    <tr>
      <th scope="row1"></th>
      <td><input type="text" placeholder="enter details" name="employee_name[]" id="employee_name"></td>
      <td> <select class="form-select" aria-label="Default select example" name="id_number[]" id="id_number" autocomplete="off">
                          <option value="">--Select--</option>
                       ...
                       <input type="text"  placeholder="enter details" name="id_details" id="id_details">

</td>
      <td><input type="text" name="total_amt[]" id="total_amt" placeholder="enter details"></td>
      <td><input type="text" name="due_amt_period[]" id="due_amt_period" placeholder="enter details"></td>
    </tr>
    @endif
                ...
                <input class="form-control" type="file" name="operational_creditor_signature" id="operational_creditor_signature" placeholder="ENTER DETAILS ..." accept="image/*" >
            ...
            <div class="tab" id="tab2">
              ...
              <table id="myTable1">
               @if(isset($other_files) && count($other_files)>0)
                ...
                <tr id="row{{$loop->index+1}}">
                  ...
                  <td><input type='file' name="other_doc_file[]" onchange="updateFile('financialCreditorFormE','/update-formE-file','{{$fl->id}}','{{$loop->index}}', '{{$tbl}}')">
              ...
              <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="del('{{$loop->index+1}}', '{{$fl->id}}', '/remove-formE-data', 'deleteRowBtn', 'financialCreditorFormE', 'errMessage_4')">Delete Row</button></td> -->
                </tr>
                @endforeach
                @else
                <tr id="row1">
             ...
             <button class="btn btn-primary" type="button" id="thirdStep" onclick="validateData2('financialCreditorFormE','thirdStep','tab2', 'tab3', check1, 'step2', 'step3', '/form/financial-creditor-form-e-save', 'errMessage_3','/form/get-formE-updated-table');declaration('declarationDiv', '{{isset($user->id) ? $user->id : ''}}', '{{isset($view) ? 'view' : ''}}', '/get-formE-signature', 'step3Sign');" >Next</button>
            ...
            <div class="tab" id="tab3">
             ...
             <br><br>
             <button class="btn btn-primary" type="button" id="prevBtn" onclick="prev('tab3','tab2','step3','step2');updateTableData('financialCreditorFormE','myTable1','/form/get-formE-updated-table');">Previous</button>
              <button class="btn btn-primary" type="button" id="secondStep" onclick="validateData('financialCreditorFormE','secondStep','tab3', 'tab4', check0, 'step3', 'step4', '/form/financial-creditor-form-e', 'errMessage_2', getPreview);" >Next</button>
