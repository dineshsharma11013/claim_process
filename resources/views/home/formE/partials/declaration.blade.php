             <h2 class="text-center">DECLARATION</h2>
              ...
              <h6>I {{isset($cat->name) ? $cat->name : ''}}, currently residing at {{isset($cat->address) ? $cat->address : $cat->address}}, do hereby declare and state as follows: - </h6><br>

              <span>{{isset($comp->name) ? ucwords($comp->name) : ''}}, the corporate debtor was, at the insolvency commencement date, being the {{dateFm3($user->insolvency_commencement_date)}} actually indebted to me for a sum of Rs. {{ $total_amt ?? ''}}.</span><br><br>
              ...
              <table id="sanctionAmtTbl">
                ...
                @if(isset($senctionDetails) && count($senctionDetails)>0)
                <table id="sanctionAmtTbl">
                <tr>  
                <td style="padding-right:30px;">
                    <label>Date</label></td>
              ...
              @foreach($senctionDetails as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionAmtTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormE', 'errMessage_4', '{{Config::get('site.formESenctionMdl')}}')">Delete Row</button></td>
                </tr>
              ...
              <table id="sanctionDeclTbl">
                ...
                @if(isset($declarationMdls) && count($declarationMdls)>0)
                <table id="sanctionDeclTbl">
                <tr>  
                <td style="padding-right:30px;">
                    <label>Date</label><br>
              ...
              @foreach($declarationMdls as $fl)
                ...
                <button class="btn btn-primary btn-sm" type="button" id="deleteRowBtn{{$loop->index+1}}" onclick="delSenction('{{$loop->index+1}}', 'sanctionDeclTbl', '{{$fl->id}}', '/remove-senction-data', 'deleteRowBtn', 'financialCreditorFormE', 'errMessage_4', '{{Config::get('site.formEDecTblMdl')}}')">Delete Row</button></td>
                </tr>
              ...
              <span>@if(isset($updated_data) && $updated_data == 'view')
              ...
              <h5>Date : @if(isset($user)) {{ ($user->submitted == 1) ? dateFm($user->date) : date("d-M-Y")}} @else {{date("d-M-Y")}} @endif</h5>
              <div>
                     ...
                     @else
                     <label>Place:</label>
                     <input type="text" name="place" id="place" value="{{isset($user->place) ? $user->place : ''}}" onKeyUp="Removefd('financialCreditorFormF', 'place')" style="margin-left: 10px;width: 250px;" autocomplete="off"><br>
              ...
              <span><img class="profile-pic-user" src="{{ asset('public/access/media/forms/formE/documents/'.$user->unique_id.'/'.$user->operational_creditor_signature) }}" width="75" height="75" style="float: right;"></span>
              ...
              <p>I {{isset($cat->name) ? $cat->name : ''}}, the claimant hereinabove, do hereby verify that the contents of his proof of claim are true and correct to my Knowledge and belief and no material fact has been concealed therefrom.</p><br>
