  ...
  <body>
    <div class="container-fluid">
        ...
        <div class="clearfix"></div>
         ...
         <div class="row">
            <p>{{!empty($user->fc_name) ? $user->fc_name : ''}} <br> {{!empty($user->fc_address) ? $user->fc_address : ''}}</p>
          ...
          <div class="row my-2">
           ...
           <div class="row">
               <h6>Madam / Sir</h6>
            <p>I {{!empty($user->fc_name) ? $user->fc_name : ''}}, hereby submits this claim in respect of the corporate insolvency resolution process of {{!empty($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
                </div>
            ...
            <div class="row">
               ...
               <p>{{!empty($user->fc_identification_number) ? $user->fc_identification_number : ''}}<</p>
               </div>
               ...
               <p>{{isset($user->claim_amt) ? 'Amount is Rs.'.$user->claim_amt : ''}} {{isset($user->claim_amt_desc) ? $user->claim_amt_desc : ''}}</p>
               </div>
...
<span class="fw-bold">{{isset($user->signing_address) ? $user->signing_address : ''}}</span>
     ...
     @if(isset($user->authorisation_letter))
      ...
      <td><a href="{{ asset('public/access/media/forms/formF/documents/'.$user->unique_id.'/'.$user->pan_number_file) }}" target="_blank">{{ $user->pan_number_file }}</a></td>
      </tr>
       @endif
                      @endif
...
<p class="text-end">@if(isset($user->fc_signature))
          ...
          <div class="container-fluid my-4 ">
            <div class="row">
                ...
                <p>I {{!empty($user->signing_person_name) ? $user->signing_person_name : $cat->name}}, the {{!empty($user->creditor_position) ? $user->creditor_position : ''}} hereinabove, do hereby verify that the contents of his proof of claim are true and correct to my Knowledge and belief and no material fact has been concealed therefrom.</p>
                <div class="row">
