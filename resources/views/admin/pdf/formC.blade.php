  ...
  <body>
    <div class="container-fluid">
        ...
        <div class="row mt-4">
            ...
            <p class="float-start"><b>From</b></p> <p class="float-end">{{ ($user->submitted == 1) ? dateFm($user->date) : date("d-M-Y")}}</p>
            </div>
        </div>
        ...
        <div class="clearfix"></div>
         ...
         <div class="row">
              <p>{{!empty($user->fc_name) ? $user->fc_name : $cat->name}}<br> {{isset($user->fc_address) ? $user->fc_address : ''}}</p>
               ...
               <h6>To</h6>
                <p>The Interim Resolution / Resolution Professional <br> {{ucwords($ip->username)}}<br>
                 {{$ip->address}}</p>
          ...
          <div class="row my-2">
           ...
           <div class="row">
               <h6>Madam/Sir</h6>
              <p>I {{!empty($user->fc_name) ? $user->signing_person_name : $cat->signing_person_name}}, hereby submits this claim in respect of the corporate insolvency resolution process of {{!empty($comp->name) ? ucwords($comp->name) : ''}}. The details for the same are set out below:</p>
          </div>
          
            <div class="row">
               <h6 style="font-size:14px;">1.NAME OF FINANCIAL CREDITOR </h6>
              <p>{{!empty($user->fc_name) ? $user->fc_name : $cat->name}}</p>
             </div>
             
              <div class="row">
               <h6 style="font-size:14px;">2. IDENTIFICATION NUMBER OF FINANCIAL CREDITOR <span>(IF AN INCORPORATE BODY PROVIDE OF INCORPORATION. IF A PARTNERSHIP OR INDIVIDUAL PROVIDE IDENTIFICATION RECORDS* OF ALL THE PARTNERS OR THE INDIVIDUAL)</span></h6>
        ...
        <div class="container-fluid my-4 border border-dark p-2">
            <div class="row">
                ...
                [ Please enclose the authority if this is being submitted on behalf of an FINANCIAL CREDITOR ]</p>
<p class="text-end">@if(isset($user->operational_creditor_signature))
          ...
          <div class="container-fluid my-4 border border-dark p-2">
            <div class="row">
                <h6 style="font-size:14px;">{{!empty($user->signing_person_name) ? $user->signing_person_name : $cat->name}}</h6>
...
<span class="fw-bold">{{isset($user->signing_address) ? $user->signing_address : ''}}</span>
         ...
         <div class="container-fluid my-2">
            <div class="row">
             <p>LIST OF DOCUMENTS ATTACHED TO THIS PROOF CLIAM IN ORDER TO PROVE THE EXISTENCE AND NON-PAYMENT OF CLAIM DUE TO THE FINANCIAL CREDITOR</p>
      ...
      @endif @endif
        ...
        <div class="row">
                ...
                <p class="fw-bold">I {{!empty($user->signing_person_name) ? $user->signing_person_name : $cat->name}}, currently residing at {{isset($user->signing_address) ? $user->signing_address : ''}}, do hereby declare and state as follows: -</p>

                <p>1. {{!empty($comp->name) ? ucwords($comp->name) : ''}}, the corporate debtor was, at the insolvency commencement date, being the {{dateFm3($user->insolvency_commencement_date)}}, actually indebted to me for a sum of Rs. {{is_numeric($total_amount) ? $total_amount : 0}}.</p>
...
<p class="text-end">@if(isset($user->operational_creditor_signature))
          ...
          <div class="container-fluid my-4 ">
            <div class="row">
                ...
                <p >I {{!empty($user->signing_person_name) ? $user->signing_person_name : $cat->name}}, the claimant hereinabove, do hereby verify that the contents of his proof of claim are true and correct to my Knowledge and belief and no material fact has been concealed therefrom. </p>
                <div class="row">
