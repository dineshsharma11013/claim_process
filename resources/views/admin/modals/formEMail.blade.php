    ...
    <section class="content">
      <div class="box box-default">
        <div class="box-header with-border">
              ...
              <h4>Mail To {{ucwords($user->name)}} </h4>
             ...
             <div class="form-group col-md-12">
              ...
              <input type="hidden" name="user_type" value="IRP">
                <div class="form-group col-md-6">
                  ...
                  <input type="text" value="{{$user->email}}" name="to" id="to" class="form-control" placeholder="Enter To" readonly>
...
<!-- <tr>
  ...
  <tr>
    <td>{{$cat->uId}}</td>
            <td>
              @if(isset($info->claim_amt))
              <span style="float: left;">{{isset($info->claim_amt) ? $info->claim_amt : ''}}</span>@endif</td> 
           
           <td>
            @if(isset($info->approved_total_amount))
            <span style="float: left;">{{isset($info->approved_total_amount) ? $info->approved_total_amount : ''}}</span>@endif</td> 
           
            <td>
              @if(isset($info->rejected_total_amount))
              <span style="float: left;">{{isset($info->rejected_total_amount) ? $info->rejected_total_amount : ''}}</span>@endif </td> 
</tr>
</table>
...
Disclaimer: Your claim has been accepted/ admitted on provisional basis based on documents and information submitted by you. We have not yet received any data from the Corporate Debtor (SIPL) to enable us to cross verify the claims with the records and books of the Corporate Debtor. Hence this admitted amount may undergo change(s) after cross verification from the appropriate data available with me from corporate debtor or any alternate authentic source. In case you have any additional corroborative information/records same may please be furnished at the earliest. 
        ...
        <div class="box-footer">
                <button type="button" onclick="sendData('RPMailForm','saveBtn','/send-formE-user-report','userModal')" id="mailBtn" class="btn btn-success">Send</button>              
