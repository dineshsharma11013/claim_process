          ...
          <div class="box">
            <div class="box-header">
             <!--  <a href="{{url(admin().'/add-user-type')}}" class="{{Config::get('site.addDataBtn')}}" style="float: right;"><i class="fa fa-plus"></i> ADD user type</a> -->
              <h3 class="box-title"><u>Form D Registered Details</u></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                ...
                @php
                  ...
                  <tr>
                    ...
                    <td>@if(isset($fm->updated_at))
                      ...
                      <a class="{{Config::get('site.detailBtn')}}" href="{{url(admin().'/form-d-registered-details/'.$fid)}}" target="_blank"><i class="fa fa-eye"></i><!--{{$us->id}}--> Details</a>

                      
                      @if(isset($cat->approval_status))
                      @if($cat->approval_status==2)
                       <button class="btn bg-maroon btn-sm" id="requestStatus_formD_{{$us->id}}" onclick="formEditApproval('{{$us->id}}','Form-D Update Request','{{$us->user_id}}', '/form-notification-update', 'requestStatus_formD_{{$us->id}}', 'Form-D')">Approve Edition</button> 
