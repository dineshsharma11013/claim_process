...
{       
    ...
    {   
       ...
       // dd($comps); die();
        ...
        $output = DB::table('form_a_mdls')
                    ->select('id','company_id','user_id', 'corporate_debtor_insolvency_date', 'insolvency_closing_date','unique_id','assign_company_mdls_id')
         ...
         $live = DB::table("form_a_mdls as fA")
                        ...
                        ->leftJoin('general_info_mdls as gIP', 'gIP.id','=','asn.ip_id')
        ...
        $upcomings = DB::table("form_a_mdls as fA")
                    ...
                    ->select('gIP.first_name as ip', 'cmp.name as company', 'asn.designation', 'fA.corporate_debtor_insolvency_date', 'fA.insolvency_closing_date', 'fA.unique_id')
                    ->where([['fA.corporate_debtor_insolvency_date','>=',$current_time]])
                    ->get();                       
       ...
       // dd($live); die();
        ...
        return view('front.front', compact("jsl","comps", "output", "live","upcomings"));
    }
    ...
    {
        $pdf = PDF::loadView('user.pdf.formAA');
        
        $now = time();
        return $pdf->stream($now.'.pdf');
    }

    function form2View($id)
    {
        $pdf = PDF::loadView('user.pdf.form2');
    ...
    function getIpDtl(Request $req, $comp)
    {
       // echo $comp;die(); 
        ...
        try
        {
            ...
            $check = DB::table('form_a_mdls')
                    ->select('id','user_id','designation')
                    ->where([['company_id','=',$comp],['status','=',1],['deleted','=',2],['corporate_debtor_insolvency_date','<=',$current_time],['insolvency_closing_date','>=',$current_time]])
                    ->orderBy('id', 'desc')
                    ->first();

            $check2 = DB::table('form_a_mdls')
                    ->select('id','user_id','designation')
           ...
           // echo print_r($data);die();        
            ...
            $ipData = DB::table('general_info_mdls')->select('first_name','id')->where([['id','=',$data->user_id],['flag','=',2],['status','=',1]])->first();
            



            $tData = DB::table("form_a_mdls as fA")
                        ->leftJoin('company_dtls as cmp', 'cmp.id','=','fA.company_id')
                        
                    ->select('fA.id as fA_ID','fA.name as ip', 'fA.designation as desg', 'fA.user_id as ip_id', 'fA.company_id as company_id', 'cmp.name as company', 'fA.corporate_debtor_insolvency_date', 'fA.insolvency_closing_date', 'fA.unique_id')
                    ->where([['fA.status','=',1],['fA.deleted','=', 2],['fA.company_id','=',$comp]])
                    ->get();


            

            $tBody = view('front.include.claimDetails', compact("tData"));        
                       
            //$ip = DB::table('general_info_mdls')->select('username','id')->where([['id','=',$data->user_id],['flag','=',2],['status','=',1]])->first();
            ...
            if ($ipData) 
            {
                ...
                $response['message'] =  $ipData->first_name;   
                $response['dtls'] = $tData;
        ...
        {
            $response['error'] = true;
            $response['message'] = $e->getMessage();//'Please refresh page and try again.';// $e->getMessage();;
        }
    ...
    function registerForm(Request $req)
    {
        ...
        //   dd($user);die();  
            ...
            $form_a = isset($user->formA) ? $user->formA : Session::get('form_a');
                $formA = DB::table("form_a_mdls")->select('id','user_id','company_id', 'corporate_debtor_insolvency_date')->where('id',$form_a)->first();
                $formA_con = DB::table("form_a_mdls")->select('id','user_id','company_id', 'corporate_debtor_insolvency_date')->where([['deleted','=',2],['corporate_debtor_insolvency_date','<=',$current_time],['insolvency_closing_date','>=',$current_time]])->where('id',$form_a)->first();    
          ...
          //  dd($user);die();
            if ($user && isset($req->edit)) 
            {
                ...
                'form_b_id' => Session::has('new_form_id') ? Session::get('new_form_id') : $user->id//$user->id//Session::has('new_form_id') ? Session::get('new_form_id') : $user->id
                ];
             ...
             //  dd($other_files);die();   
                return view('home.operational_creditor_form_b_edit', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","notify","comp","edit", "formA"));    
              //  dd($other_files);
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
                ...
                if (Session::has('form_a')) 
                {
                    ...
                    //echo print_r($formA);die();   
                        ...
                        return view('home.operational_creditor_form_b', compact("jsl","cat","irp","user","other_files","user_type","ar","comp", "formA"));    
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
                    }
            ...
            $jsl = bPath().'/'.Config::get('site.operationalCreditorV');
               ...
               // dd($form_a); die();
                ...
                return view('home.operational_creditor_form_b_view', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","comp","view", "reg", "formA"));    
            }
            else
            {
                if (Session::has('form_a')) 
                {
                    ...
                    if ($formA_con) 
                    {
                        ...
                        //dd("sdf");
                        return view('home.operational_creditor_form_b', compact("jsl","cat","irp","user_type","ar","comp", "formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
        ...
        {  
           ...
           // dd($user);die();  
            ...
            return view('home.financial_creditor_form_c_edit', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","notify","comp","edit", "formA"));
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
             ...
             $where = [
                ...
                $other_files = $this->getRecords('form_c_other_document_mdls',$where);
               // dd($user); die();  
                return view('home.financial_creditor_form_c', compact("jsl","cat","irp","user","other_files","user_type","ar","comp", "formA"));
           ...
           // dd($user); die();  
            return view('home.financial_creditor_form_c_view', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","comp","view", "reg", "formA"));
            }
            ...
            else
            {
                ...
                if (Session::has('form_a')) 
                { 
                    $formA = DB::table("form_a_mdls")->select('id','user_id','company_id')->where([['deleted','=',2],['corporate_debtor_insolvency_date','<=',$current_time],['insolvency_closing_date','>=',$current_time]])->where('id',Session::get('form_a'))->first();
                    ...
                    //dd($cat); die();    
                    return view('home.financial_creditor_form_c', compact("jsl","cat","irp","user_type","ar","comp", "formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
        ...
        elseif($req->type=='financial-creditor-in-a-class'  && $req->name == $cat->name && $req->email == $cat->email && $req->mobile == $cat->mobile)
        {
           ...
           // dd($user);die();
            ...
            return view('home.financial_creditor_form_ca_edit', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","notify","comp","edit", "formA"));
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
                $where = [
                'user_id' => $user_id,
                'unique_id' => $user->unique_id,
                'form_ca_id' => Session::has('new_form_id') ? Session::get('new_form_id') : $user->id//$user->id
            ];
            ...
            if (Session::has('form_a')) 
            {
                ...
                return view('home.financial_creditor_form_ca', compact("jsl","cat","irp","user","other_files","user_type","ar","comp", "formA"));
                }
                else
                {
                    return redirect()->back()->with("danger","Date is closed now.");
            ...
            { 
                ...
                'user_id' => $user_id,
                'unique_id' => $user->unique_id,
                'form_ca_id' => $user->id//Session::has('new_form_id') ? Session::get('new_form_id') : $user->id
            ];
            ...
            $view = isset($req->view) ? $req->view : '';
                ...
                $other_files = $this->getRecords('form_ca_other_document_mdls',$where);  
                return view('home.financial_creditor_form_ca_view', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","comp","view", "reg", "formA"));
            }
            else
            {
                ...
                if (Session::has('form_a')) 
                {
                    ...
                    return view('home.financial_creditor_form_ca', compact("jsl","cat","irp","user_type","ar", "comp", "formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
        ...
        elseif($req->type=='workmen-and-employee'  && $req->name == $cat->name && $req->email == $cat->email && $req->mobile == $cat->mobile)
        {
            ...
            //dd($comp);die();
            return view('home.workmen_and_employee_form_d_edit', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","notify","comp","edit", "formA"));
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
                $where = [
            ...
            if ($formA_con) 
            {
                ...
                $other_files = $this->getRecords('form_d_other_document_mdls',$where);  
                return view('home.workmen_and_employee_form_d', compact("jsl","cat","irp","user","other_files","user_type","ar","comp", "formA"));
            }
            else
            {
                return redirect()->back()->with("danger","Date is closed now.");
            ...
            elseif ($user && isset($req->view)) 
            {
                ...
                'user_id' => $user_id,
                'unique_id' => $user->unique_id,
                'form_d_id' => Session::has('new_form_id') ? Session::get('new_form_id') : $user->id
            ];
                }
           ...
           // dd($view);die();
            return view('home.workmen_and_employee_form_d_view', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","comp","view", "formA"));
            }
            ...
            else
            {
                if (Session::has('form_a')) 
                {
                    ...
                    if ($formA) 
                    {
                        ...
                        //echo print_r($irp);die;
                        return view('home.workmen_and_employee_form_d', compact("jsl","cat","irp","user_type","ar","comp", "formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
        ...
        if (isset($req->view) || isset($req->edit)) 
          {
           ...
           // dd($user);die();
            ...
            return view('home.authorised_representative_of_workmen_and_employee_form_e_edit', compact("jsl","cat","irp","user","other_files","emps","edit_data","user_type","ar","notify","comp","edit", "formA"));
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
                $where = [
            ...
            if ($formA_con) 
            {
                ...
                $irp = DB::table("general_info_mdls")->select('id','username','email','mobile','address')->where('id',$formA->user_id)->first();
                    ...
                    $emps =  $this->getRecords('form_e_employee_detail_mdls',$where);
                //dd($emps);
                return view('home.authorised_representative_of_workmen_and_employee_form_e', compact("jsl","cat","irp","user","user_type","ar","emps","other_files","comp", "formA"));
            }
            else
            {
                return redirect()->back()->with("danger","Date is closed now.");
            ...
            //dd($view);die();
            return view('home.authorised_representative_of_workmen_and_employee_form_e_view', compact("jsl","cat","irp","user","other_files","emps","user_type","ar","edit_data","reg","comp", "view", "formA")); 
            }
            else
            {
                if (Session::has('form_a')) 
                {
                   ...
                   // dd($formA);die();
                    return view('home.authorised_representative_of_workmen_and_employee_form_e', compact("jsl","cat","irp","user","user_type","ar", "formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
        ...
        elseif($req->type=='other-creditor' && $req->name == $cat->name && $req->email == $cat->email && $req->mobile == $cat->mobile)
        {
          ...
          // dd($user);die(); 
           ...
           // dd($user);die();
            ...
            return view('home.other_creditor_form_f_edit', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","notify","comp","edit", "formA"));
            }
            elseif ($user && !isset($req->view) && !isset($req->edit)) 
            {
                $where = [
            ...
            if (Session::has('form_a')) 
             ...
             if ($formA_con) 
             {
                ...
                $other_files = $this->getRecords('form_f_other_document_mdls',$where);  
                return view('home.other_creditor_form_f', compact("jsl","cat","irp","user","other_files","user_type","ar","comp","formA"));   
                }   
                else
                {
                    return redirect()->back()->with("danger","Date is closed now.");
           ...
           // dd($other_files);die();
            return view('home.other_creditor_form_f_view', compact("jsl","cat","irp","user","other_files","user_type","ar","edit_data","comp","view", "reg", "formA"));   
            }
            else
            {
                ...
                if (Session::has('form_a')) 
                {
                    ...
                    //dd($cat); die();    
                        return view('home.other_creditor_form_f', compact("jsl","cat","irp","user_type","ar","comp","formA"));
                    }
                    else
                    {
                        return redirect()->back()->with("danger","Date is closed now.");
    ...
    function userSignUp(Request $req, UsersSignUp $signUp)
    {
        $response = array();
        $data = [
            'admin_id'=>Config::get('site.unauthourised'),
            'name' => $req->name ?? "",
            'email' => $req->email ?? "",
            'mobile' => $req->mobile ?? "",
            'alt_mobile'=>'',
        ...
        $authCheck = $this->signUpAuth(Config::get('site.authMdl'));
        ...
        if ($check) 
        {
            //echo $check->unique_id;die();
            if ($check->status == 1 && $check->auth_check == 1) 
            {
                $response['error'] = true;
                $response['message'] = "1 You are already registered. Please login.";  
                echo json_encode($response);
                die(); 
            }
            elseif ($check->status == 2 && $check->auth_check == 1)
            {
                $response['error'] = true;
                $response['message'] = "You are not authourized to login.";//"Email-Id/mobile number already exists. Please try another.";   
                echo json_encode($response);
                die();
            }
            elseif ($check->auth_check != 1 && $check->status == 2) 
            {
                if ($authCheck) 
                  {
                    if ($authCheck->type=='both' || $authCheck->type=='email' && $authCheck->status=='yes') 
                    {
                ...
                {
                        $userCK = DB::table(Config::get('site.userMdl'))->where('unique_id', $req->unique_id)->update(['status' => 1, 'auth_check' => 1, 'date' => date('Y-m-d')]);
            
                        if ($userCK) 
                        {
                            $cat = DB::table("user_mdls")->where('unique_id',$unique_id)->orderBy('id','desc')->first();
            ...
            {
                $response['error'] = true;
                $response['message'] = "Something went wrong. Please contact to the administrator.";
            }
        ...
        else
        {
          ...
          if ($authCheck) 
          {
            if ($authCheck->type=='both' || $authCheck->type=='email' && $authCheck->status=='yes') 
            {
                ...
                if ($cat) 
                {
                    $cat3 = DB::table("user_mdls")->where('unique_id',$req->unique_id)->orderBy('id','desc')->first();
        ...
        else
        {
            ...
            $this->insertData(Config::get('site.userMdl'),$data); 
            $userCK = DB::table(Config::get('site.userMdl'))->where('unique_id', $req->unique_id)->update(['status' => 1, 'auth_check' => 1, 'date' => date('Y-m-d')]);
...
function signUpResend(Request $req, UsersSignUp $signUp)
{
    $response = array();
...
function userSignIn(Request $req)
{
    $response = array();
        ...
        $com = DB::table("general_info_mdls")->where([['user_type','=',1],['sub_user','=',""]])->first();
       // $auth = DB::table("authentication_mdls")->select('type','status')->orderBy('id','desc')->first();
        $check = DB::table("user_mdls")->select('id','name','email','auth_type','unique_id','mobile','password','pwd', 'admin_id')->where([['unique_id',$email],['status',1],['auth_check',1]])->orderBy('id', 'desc')->first();
