...
class formECntrlr extends Controller
{
	...
	function formERegistration(Request $req)
    {
        ...
        else
        {
            ...
            $chek =  $this->getFirstRow(Config::get('site.formEMdl'),['id'=>$req->fid]);
            if ($chek) 
            {
               $Bunique_id = $chek->unique_id; 
            }
            else
            {
                $Bunique_id = $unique_id;     
      ...
      //   	];
        ...
        $data = [
            'company' => '',//Session::has('company_id') ? Session::get('company_id') : '',
            'irp' => '',///Session::has('ip_id') ? Session::get('ip_id') : '',
        ...
        try
        {
            if ($check) 
            {
                ...
                $data2 = [
                 ...
                 $check2 = DB::table(Config::get('site.formEMdl'))->where([['unique_id','=', $Bunique_id],['user_id','=', $user_id]])->latest()->first();      
               ...
               // $this->formEEmplFields(Config::get('site.formEEmpDtl'),$where2, $check2->id, $check->form_e_selected_id);
                ...
                $this->formEOtherFields(Config::get('site.formEOtherDoc'),$where2, $check2->id, $check->form_e_selected_id);
...
function formEOtherFields($tbl, $where, $newId, $selected)
    {
...
function formEEmplFields($tbl, $where, $newId, $selected)
    {
        $records = $this->getRecords($tbl, $where);
        ...
        { 
            $data=[
                ...
                'id_details' => $rc->id_details,
                'total_amt' => $rc->total_amt,
                'due_amt_period'=>$rc->due_amt_period,
                'submitted'=>2,
                'unique_id'=>$rc->unique_id,
                'rem_addr' => $rc->rem_addr,
                'created_at' => date('Y-m-d H:i:s'),
    ...
    {
        $user_id = Session::get('user_id'); 
        $fm_id = Session::get('new_form_id') ? Session::get('new_form_id') : $req->fid;//Session::get('new_form_id');
        $output = "";

        //echo print_r($fm_id);die();
        $cat = DB::table("user_mdls")->select('id','name','email','mobile','address')->where('id',$user_id)->first();
        if ($req->updated_data == 'edit') 
        {
            $user = DB::table('form_e_file_mdls');
            $user = $user->where([['form_e_file_mdls.id','=',$fm_id]])->first();
        }
        elseif ($req->updated_data == 'view')
        {
            $user = DB::table('form_e_file_mdls');
        ...
        $output = view('home.formE.partials.declaration', compact("user","cat","comp","total_amt"));
        
        

       echo $output;
      // echo print_r($total_amt);
    }
               ...
               if ($req->notify) 
                {
                    ...
                    $response['error'] = false;
                    $response['cls'] = 'success';
                    $response['message'] = $cat->id." Form updated successfully.";
                }
                else
                {     
...
function getFormEUpdatedTable(Request $req)
    {
        ...
        $other_files = $this->getRecords(Config::get('site.formEOtherDoc'),['form_e_id'=>$check->id]);
    ...
    //echo print_r($user);die();
      //  echo $req->updated_data;die();
        ...
        $output = view('home.formE.partials.preview', compact("cat","user","other_files","emps","ip","comp","total_amt"));
        echo $output;
