...
class assignCompanyCntrlr extends Controller
{
  use MethodsTrait;
    ...
    function saveAssignIp(Request $req)
    {
      ...
      try
        {
        $cat2 = DB::table('assign_company_mdls')->where('company_id', $req->company)->orderBy('id','desc')->first();
        if ($cat2) 
        {
          if ($cat2->ip_id==$req->ip) 
          {
                $data2= [
                'appointment_date' => $req->appointment_date ?? "",
                'designation' => $req->designation ?? "",
                'status' => $req->status, 
                'termination_date' => $req->designation == 'AR' ? date('Y-m-d') : "",
                'update_time' => date('Y-m-d H:i:s'),
                'updated_by' => Session::get('admin_id'),
                'rem_addr' => $req->ip()
              ];

              DB::table('assign_company_mdls')->where('id',$cat2->id)->update($data2);

            
          }
          else
          {
            // if ($cat2->termination_date=='') 
            // {
            //   $data2= [
            ...
            //   }
              ...
              DB::table('assign_company_mdls')->where([['company_id','=',$req->company],['designation','=',]])->update($data2);
            
              $this->insertData('assign_company_mdls',$data);
          }    
        }
        else
        {
          ...
          {
            $response['error'] = true;
            $response['message'] = $e->getMessage();//"IP Not Assigned. Try Again Later.";
          }
       // }
    ...
    function updateAssignIp(Request $req, $id)
    {
      ...
      if ($check) 
      {
        $data2= [
          //'designation' => $req->designation ?? "",
          'appointment_date' => $req->appointment_date ?? "",
          'termination_date' => $req->status == 1 ? "" : date('Y-m-d'),
          'status' => $req->status, 
          'update_time' => date('Y-m-d H:i:s'),
          'updated_by' => Session::get('admin_id'),
          'rem_addr' => $req->ip()
       ...
       if ($check->company_id == $req->company && $check->ip_id == $req->ip && $check->designation == $req->designation) 
       {
